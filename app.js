    ...
    .map((record) => {
      ...
      return `
        <li class="record-item">
          <div><strong>${escapeHtml(record.detail)}</strong></div>
          <div class="meta">${escapeHtml(record.category)} ｜ ${escapeHtml(record.phase)} ｜ ${timeText}</div>
          ${noteHtml}
        </li>
      `;
    })
